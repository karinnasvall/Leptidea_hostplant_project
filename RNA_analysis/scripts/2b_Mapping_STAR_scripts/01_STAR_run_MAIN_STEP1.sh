...
# Performs read mapping with STAR and transcript quantification with StringTie (in the next step)
...
RR=/proj/uppstore2017185/b2014034_nobackup/Veronika/RNA_HostPlantProject/results/2b_Mapping_STAR_STEP1
...
for i in `seq 1 1 $N_SAMPLES`; do
   ...
   sbatch $SRCDIR_INI/01_STAR_run_query_STEP1.sh $AA/$RNAFILE_A \
                                                 $AA/$RNAFILE_B \
                                                 ${RNAFILE_A} \
                                                 ${RNAFILE_B} \
                                                 ${RR}/${GO_SAMPLE} \
