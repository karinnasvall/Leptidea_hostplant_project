...
AA=/proj/uppstore2017185/b2014034_nobackup/Veronika/RNA_HostPlantProject/results/1d_prinseq

# Path to output folder
RR=/proj/uppstore2017185/b2014034_nobackup/Veronika/RNA_HostPlantProject/results/1e_cutadapt
...
for i in `seq 1 1 $N_LIBS`; do
   READS[$i,1]=${SAMPLE_LIST[$i]}_prinseq_R1.fastq.gz
   READS[$i,2]=${SAMPLE_LIST[$i]}_prinseq_R2.fastq.gz
done
...
for i in `seq 1 1 $N_LIBS`; do
   ...
   sbatch 01_cutadapt_run_query.sh $AA/$RNAFILE_A \
                                               $AA/$RNAFILE_B \
                                               ${RNAFILE_A} \
                                               ${RNAFILE_B} \
                                               $RR \
